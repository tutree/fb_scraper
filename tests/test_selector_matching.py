...
def card_page(browser: Browser) -> Page:
    ...
    # Wrap the raw card fragment in the same feed+article structure that exists
    # on real Facebook search result pages.
    wrapped = f'<div role="feed"><div role="article" aria-label="Test post">{CARD_HTML}</div></div>'
...
def write_results(browser: Browser):
    ...
    wrapped = (
        f'<div role="feed"><div role="article" aria-label="Test post">'
        f'{CARD_HTML}</div></div>'
    )
    cpage.set_content(wrapped, wait_until="domcontentloaded")
    ...
    post = cpage.evaluate("""
        () => {
            ...
            return { posterName, profileUrl, commentCount, likeCount, shareCount };
        }
    """)
    cpage.close()
